...
- 环境基准对照：服务端接入 MetaTFT 公开接口，基于前四率、登顶率、均名和样本量计算版本强势阵容
- 逐局展开复盘：查看每局名次、时间、等级、羁绊骨架、关键单位、终盘英雄面板与复盘建议
- 完整对局面板：支持查看终盘英雄、装备、羁绊、强化符文、最后回合和已知节点时间线
...
- 服务端：Node.js `http` 模块
- 数据源：Riot TFT API、MetaTFT 公开接口、Riot Data Dragon TFT 静态资源
...
- 逐局展开复盘
- 完整对局面板
  展示终盘英雄、装备、羁绊、强化符文、最后回合与已知节点时间线
...
- Riot TFT 历史接口不提供逐回合时间线，当前只能展示终盘快照与已知节点
- 个别英雄、装备或羁绊在演示数据里可能因为不是当前 Set 资源而回退成字母占位
- 环境基准依赖第三方公开接口，稳定性仍受外部数据源影响
